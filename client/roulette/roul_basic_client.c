      ...
      *place = msg_rcv->room.place[place_id];  // обновление своего поля
      ...
      switch (msg_rcv->code_msg) {
        ...
        case RESULT_ROUL:
          ...
          print_table();
          set_color_roul(get_colour(msg_rcv->room.result.value));
          set_color_roul(0);
...
void get_bet_roul(place_roul_t* place) {
  ...
  while (place->count_bet != MAX_NUM_BET &&
    ...
    do {
      clean_tty();
      printf("Your balance: %d \U0001F4B8\n", place->player.balance);
      print_table();
      print_list_bet(place->count_bet);
      ans = get_ans_int_roul();
    ...
    if (ans > 6) {
      ...
      switch (ans) {
        case 7:
          printf("1.First\n2.Second\n3.Third\n");
          do {
            input_char_roul(buff, sizeof(buff));
