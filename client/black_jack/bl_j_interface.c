...
static const char *MENU_[5][4] = {
    {"Continue", "Insurance", "Give up", "\0"},
    {"More", "Enough", "Double", "Split"},
    {"Black Jack", "Exit", "\0", "\0"},
    {"\u2705 Stay in room", "\u274C Leave room", "\0", "\0"},
    {"\u2705 Ready", "\u274C Leave room", "\0", "\0"}};

char *get_char_status(const place_t *place, char *status) {
  status = (place->status == PLAEYR_PLAY) ? GREEN "in game" RESET
           : (place->status == END_GAME)  ? YELLOW "finished" RESET
...
void print_main_menu(const place_t *place, int index, int my_place_id) {
  if (index < DILER_PLACE) {
    printf("\nPlace #%d \U0001F47B %s             %s\n First hand: ", index,
           place->client.name,
           (index == my_place_id) ? GREEN "<--ME" RESET : "\0");
    // Вывод карт 1ой руки
    ...
    if (place->next_insur_giveup == INSURANCE) {
      printf(" Insurance bet: %d %s", place->pl_bet.insurance,
             (place->pl_bet.insurance == 0) ? YELLOW "bet lost" RESET : "\0");
    };

    if (index < DILER_PLACE) {
      char status[30];
      printf("\n Status: %s \n", get_char_status(place, status));
    }
    printf("balance = %d \U0001F4B8\n", place->client.balance);
...
int choice_menu(int size_menu, int code_menu, room_t *room, int my_place_id) {
  ...
  while (running) {
    ...
    // Получаем ввод
    char c = getchar();
    if (c == 27) {  // ESC означает начало последовательности
      getchar();            // Пропускаем '['
      switch (getchar()) {  // Читаем направление стрелки
