...
{   
    ...
    switch (msg_rcv->msg_code)
    {
        ...
        case REPLY_CHOICE_ROOM:
            ...
            {
                if (get_num_player(room->place) == NUM_PLACE || room->game_status == GAME_IN_PROCCESS)
                {   
                    send_room(room, socket, 1); // Комната недоступна
                    return;
                }
                if (room->place[k].client.socket == 0) // посадка игрока на место
                {
                    init_player(&room->place[k].client, socket);
                    room->place[k].status = PLAEYR_PLAY;
                    printf("Room is %d/5, num_player = %d sock = %d\n", k + 1, get_num_player(room->place), socket);
                    msg_rcv->id_place = k;
                    break;
                }
            }
            ...
            printf("Add pl in room #%d numplayer = %d\n", room->id, room->num_player);
        ...
        case DISCONNECT_PLAYER:
            ...
            if(room->game_status == GAME_WAITING_START)
                break;
        ...
        case LEAVE_ROOM:
            printf("Player place[ %d ] socket [%d] leave room [ %d ]\n", place_id, socket, room->id);

                for(int i = 0; i < NUM_PLACE; i++)
                {
                    printf("place #%d socket %d\n", i, room->place[i].client.socket);
                }
            ...
            {   
                printf("New place %d socket %d rooom_id = %d\n", index, room->place[index].client.socket, room->id);
                send_msg_room(room->place[index].client.socket, room, index, NEW_PLACE_ID_MSG_BL);
                index++;
            }
        ...
        printf("COUNT = %d, num_player %d\n", room->count[2], get_num_player(room->place));
        if (room->count[2] == get_num_act_pl(room) && room->game_status == GAME_WAITING_START && get_num_act_pl(room) > 0) // Выполнилось условие старта
        {
            printf("Запуск игры\n");
            room->count[2] = 0;
            room->num_player = get_num_player(room->place);
        ...
        {   
            if (msg_rcv->place.client.socket != 0 || msg_rcv->place.status == DISCONNECTED)
            {   
                room->place[place_id] = msg_rcv->place; // Обновление room
            }

            printf(" act = %d, status = %d, code = %d\n", get_num_act_pl(room), room->game_status, msg_rcv->msg_code);
            if ((get_num_act_pl(room) == 0) && (room->game_status == GAME_IN_PROCCESS) && (msg_rcv->msg_code == DISCONNECT_PLAYER))
            {
...
void select_1_reply(room_t *room)
{
    ...
    if(room->count[1] >= get_num_act_pl(room))
    {
        ...
        for (int i = 0; i < room->num_player; i++)
        {
            ...
            {   
                if(room->place[i].client.socket != 0)
                    send_msg_room(room->place[i].client.socket, room, i, REQUEST_SELECT_2);
            }
        }
    }
}
...
void more_reply(room_t *room, int place_id)
{
    ...
    if(k == 0)
    {
        send_msg_room(room->place[place_id].client.socket, room,  place_id, REQUEST_SELECT_2);
    }
...
{  
    ...
    {   
        if (diler_end_game(&room->place[DILER_PLACE], DILLER_RULE, room->deck, card_account) == 0)
        {
            for (int i = 0; i < room->num_player; i++)
            {
                if(room->place[i].client.socket != 0)
...
{   
    ...
    for (int i = 0; i < room->num_player; i++)
    {
        send_msg_room(room->place[i].client.socket, room,  i, END_ROUND);   //вопрос прододжить в этой комнате
    }
    int id = room->id;
...
void black_jack_result(room_t *room, int i)
{
    ...
    {   
        if (room->place[DILER_PLACE].sum[FIRST_HAND] > 21 || room->place[DILER_PLACE].sum[FIRST_HAND] < room->place[i].sum[FIRST_HAND])
        {   
            // win
        ...
        else if (room->place[DILER_PLACE].sum[FIRST_HAND] > room->place[i].sum[FIRST_HAND])  // поражение
        {   
            // lose
            room->place[i].win_status = 3;
            room->place[i].pl_bet.start = 0;
        }
        else if (room->place[DILER_PLACE].sum[FIRST_HAND] == room->place[DILER_PLACE].sum[FIRST_HAND])  //ничья
        {
            // draw
...
void send_room(room_t *room, int socket, int flag)
{
    ...
    for (int i = 0; i < NUM_PLACE; i++)
    {
        ...
        printf("place [%d] sock [%d] stat [%d]\n", i, room->place[i].client.socket, room->place[i].status);
    }
    send(socket, (void *)&info_room, sizeof(msg_info_room), 0);
    printf("Send Rooms\n");
}
