...
void send_msg_room_roul(int socket, const room_roul_t *room, int place_id,
  ...
  *(msg_room_roul_t *)main_msg.data = msg;
  int bytes = 0;
  if(socket != SOCKET_CHECK)
    bytes = send(socket, &main_msg, sizeof(struct MainMessage), 0);
  printf("SEND bytes %d sock = %d Code = %d\n", bytes, socket ,code_msg);
}
...
void send_msg_place_roul(int socket, const place_roul_t *place, int place_id,
  ...
  *(msg_place_roul_t *)main_msg.data = msg;
  int bytes = send(socket, &main_msg, sizeof(struct MainMessage), 0);
  if (bytes <= 0) {
    fprintf(stderr, "Error sending message place: %s\n", strerror(errno));
    close(socket);
  }
  printf("SEND bytes %d Code = %d\n", bytes, code_msg);
}
...
void send_msg_info_client(int socket, int choice)
{
    ...
    int a = send(socket, (void *)&snd_msg, sizeof(struct MainMessage), 0);
    if (a <= 0) {
      fprintf(stderr, "Error sending message info room: %s\n", strerror(errno));
      close(socket);
    }
    printf("SEND bytes %d Code = %d\n", a, REPLY_CHOICE_ROOM_ROUL);
}