...
void loop_server_roulette(room_roul_t *room, msg_place_roul_t *msg_rcv,
  ...
  switch (msg_rcv->msg_code) {
    ...
    case REPLY_CHOICE_ROOM_ROUL:
      for (int k = 0; k < MAX_PLACE_ROUL; k++)  // Клиент - место
      {
        ...
        if (room->place[k].player.socket == 0)  // !!!!ЗАМЕНИТЬ НА NULL
        {
          ...
          init_player(&room->place[k].player, socket);
          msg_rcv->place_id = k;
     ...
     case DISCONNECTED_ROUL:
      ...
      if (room->count[0] == get_num_player_roul(room->place))
        msg_rcv->msg_code = REPLY_BET_ROUL;
      break; 
    ...
    case LEAVE_ROOM_ROUL:
      ...
      {
        send_msg_room_roul(room->place[index].player.socket, room, index, NEW_PLACE_ID_MSG_ROUL);
        index++;
      }
      ...
      if(room->count[2] == get_num_player_roul(room->place) && room->status == ROOM_OPEN && get_num_player_roul(room->place) != 0)
      {
    ...
    { 
      ...
      { 
        case BET_TYPE_BLACK:
          ...
          {
            room->place[i].player.balance += 2*room->place[i].bet[k].bet_value;
        ...
        break;
         case BET_TYPE_DOZEN:
          if((room->result.value - 1) % 12 == room->place[i].bet[k].parametr - 1)
          {
            room->place[i].player.balance += 3*room->place[i].bet[k].bet_value;
            room->place[i].bet_status[k] = WIN;
                   }else{
            room->place[i].bet_status[k] = LOSE;
          }
        break;
         case BET_TYPE_STRAIGHT:
          ...
          {
            room->place[i].player.balance += 36*room->place[i].bet[k].bet_value;
            room->place[i].bet_status[k] = WIN;
          }else{
            room->place[i].bet_status[k] = LOSE;
          }
      ...
      //room->place[i].bet[k].bet_value = 0; // Сгораниие ставки
...
{
  strcpy(player->name, "client_name");
  player->balance = 100;
...
void room_balance_roul(place_roul_t *place)
{
  ...
  void send_room_roul(room_roul_t *room, int socket, int flag) {
    ...
    *(msg_info_room_roul_t *)main_msg.data = info_room;
    ...
    CHECK_ERROR(send(socket, (void*)&main_msg, sizeof(struct MainMessage), 0), -1);
  }